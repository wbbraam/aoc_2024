...
fn calculate_antinode(antenna: Vec<(i32, i32)>, maxcol: usize, maxrow: usize) -> Vec<(i32, i32)> {
    ...
    let mut t = 0;
    let mut antinode: Vec<(i32, i32)> = antenna.clone();
    ...
    for i in 0..antenna.len() {
        ...
        for j in i+1..antenna.len() {
            ...
            let mut anti1x: i32 = a1x;
            let mut anti2x: i32 = a2x;
            let mut anti1y: i32 = a1y;
            let mut anti2y: i32 = a2y;
            ...
            while go {
                ...
                if a1x < a2x {
                    anti1x = anti1x - dx.abs();
                    anti2x = anti2x + dx.abs();
                } else {
                    anti1x = anti1x + dx.abs();
                    anti2x = anti2x - dx.abs();
                }
                ...
                if a1y < a2y {
                    anti1y = anti1y - dy.abs();
                    anti2y = anti2y + dy.abs();
                } else {
                    anti1y = anti1y + dy.abs();
                    anti2y = anti2y - dy.abs();
                } 
